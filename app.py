...
def get_service_line1(group_df):
    ...
    google  = normalize_text("App ID 97")
    apple   = normalize_text("App ID 79")
    topup   = normalize_text("App ID 52")
...
def mock_reply_ticket(ticket_id):
    ...
    if not attachment_name and has_attachment:
        f = request.files.get("attachments[]")
        attachment_name = f.filename if f else ""
...
@app.route("/mock-freshdesk/api/v2/tickets/<ticket_id>/replies", methods=["GET"])
def mock_get_replies(ticket_id):
    ticket = MOCK_DB.get(str(ticket_id))
    if not ticket:
        return jsonify({"error": "Ticket not found"}), 404
    return jsonify({"replies": ticket["replies"]})
...
/* ── Topbar ── */
.topbar{position:sticky;top:0;z-index:100;height:64px;background:#000000;display:flex;align-items:center;padding:0 24px;box-shadow:0 2px 12px rgba(0,0,0,.6)}
.topbar::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,#0D3CC9 0%,#00A6FF 50%,#08D26F 100%)}
.topbar-title{position:absolute;left:50%;transform:translateX(-50%);font-size:19px;font-weight:800;color:#ffffff;white-space:nowrap;letter-spacing:0px;font-family:Arial,sans-serif}
/* ── Stepper ── */
...
<!-- Topbar -->
<div class="topbar">
  <div class="topbar-title">Zalopay Dispute Automation Agent</div>
</div>
...
<div class="wrap">
  ...
  <div class="page" id="pg-3">
    <div class="card">
      ...
      <div style="margin-bottom:10px">
        <input id="s3search" type="text" placeholder="🔍 Tìm theo số ticket..." oninput="filterTickets()"
  ...
  <div class="page" id="pg-4">
    <div class="card">
      ...
      <div id="s4body"></div>
      <!-- Bảng ticket list + reply dropdown -->
      <div id="s4ticketWrap" style="display:none;margin-top:20px">
        <div style="font-weight:600;font-size:14px;color:#003087;margin-bottom:10px">📋 Danh sách ticket đã gửi</div>
        <div style="margin-bottom:10px">
          <input id="s4search" type="text" placeholder="🔍 Tìm theo số ticket..." oninput="filterS4()"
            style="width:100%;box-sizing:border-box;padding:8px 12px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none">
        </div>
        <div id="s4ticketList"></div>
      </div>
    </div>
  </div>
...
<script>
let selFile = null, res = null, currentPage = 1, maxStep = 1;
...
function goTo(n) {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    if (i > n && i <= maxStep) { si.classList.add('done'); document.getElementById('sn-'+i).textContent = '✓'; }
    if (i > maxStep) { document.getElementById('sn-'+i).textContent = String(i); }
    // Cho click tất cả các bước đã từng đến
    const target = i;
    if (i <= maxStep) {
      si.style.cursor = 'pointer';
      si.onclick = () => goTo(target);
    } else {
...
function switchTab(name, ev) {
  ...
  if (name === 'tickets') loadTickets().then(filterTickets);
}
...
function buildStep2(d) {
  ...
  document.getElementById('s2body').innerHTML = html + '<div id="s2table"></div><div id="s2pager" style="display:flex;align-items:center;justify-content:center;gap:10px;margin-top:12px"></div>';
  s2Page = 0;
  renderS2Page();
}
function renderS2Page() {
  ...
  const totalPages = Math.ceil(total / S2_PAGE_SIZE);
  const start = s2Page * S2_PAGE_SIZE;
  const slice = files.slice(start, start + S2_PAGE_SIZE);
  let t = '<table><thead><tr><th>File Word</th><th>Bank</th><th>Ticket ID</th><th>Số GD</th><th>Trạng thái</th></tr></thead><tbody>';
  for (const f of slice) {
    const b = f.ready.toUpperCase() === 'YES'
      ? '<span class="badge badge-ok">Sẵn sàng</span>'
      : '<span class="badge badge-skip">Chưa YES</span>';
    t += `<tr><td>📄 ${f.file_name}</td><td>${f.bank}</td><td>#${f.ticket_id||'-'}</td><td>${f.num_rows}</td><td>${b}</td></tr>`;
  }
  t += '</tbody></table>';
  document.getElementById('s2table').innerHTML = t;
  const pager = document.getElementById('s2pager');
  pager.innerHTML = `
    <button class="btn btn-gray" style="padding:5px 12px" onclick="s2Page--;renderS2Page()" ${s2Page===0?'disabled':''}>← Trước</button>
    <span style="font-size:13px;color:#64748b">Trang ${s2Page+1} / ${totalPages} &nbsp;(${total} file)</span>
    <button class="btn btn-gray" style="padding:5px 12px" onclick="s2Page++;renderS2Page()" ${s2Page>=totalPages-1?'disabled':''}>Tiếp →</button>
  `;
  document.getElementById('s2body').scrollTop = 0;
  document.getElementById('s2act').innerHTML = `
    <button class="btn btn-gray" onclick="downloadAll()">⬇️ Tải ZIP file Word</button>
    <button class="btn btn-blue" id="btn-next" onclick="convertAndNext(${s2Ready})">Tiếp tục gửi Freshdesk (${s2Ready} ticket) →</button>
  `;
...
function buildStep3() {
  ...
  for (const f of ready) {
    html += `<div class="preview-email ticket-row" id="row-${f.ticket_id}">
      ...
      <span style="color:#003087;font-size:12px"><b>Bộ phận vận hành Zalopay</b> · Operations Team · E: chargebackzp@vng.com.vn · P: (028) 3962 3888 ext:3227</span>
      <div><span class="preview-attach" style="cursor:pointer" onclick="window.open('/view/${encodeURIComponent(f.file_name)}','_blank')">📎 ${f.file_name.replace(/[.]docx$/i,'.pdf')}</span></div>
    </div>`;
  }
...
function updateSendBtn() {
  ...
  const selBtn = allChecked
    ? `<button class="btn btn-gray" onclick="toggleAllBtn(false)"><input type="checkbox" checked style="width:18px;height:18px;accent-color:#003087;pointer-events:none;vertical-align:middle;margin-right:6px"> Bỏ chọn tất cả</button>`
    : `<button class="btn btn-gray" onclick="toggleAllBtn(true)"><input type="checkbox" style="width:18px;height:18px;accent-color:#003087;pointer-events:none;vertical-align:middle;margin-right:6px"> Chọn tất cả</button>`;
  ...
  document.getElementById('s3act').innerHTML =
    `<span>${count}</span><span style="margin-left:auto;display:flex;align-items:center;gap:8px">
      <button class="btn-ghost" onclick="goTo(2)" style="font-size:13px">← Quay lại</button>
      ${send}
    </span>`;
  const selEl = document.getElementById('s3sel');
  if (selEl) selEl.innerHTML = selBtn;
}
function filterTickets() {
  const q = (document.getElementById('s3search').value || '').trim().toLowerCase();
  document.querySelectorAll('#s3body .ticket-row').forEach(row => {
    const tid = (row.id || '').replace('row-','').toLowerCase();
    row.style.display = (!q || tid.includes(q)) ? '' : 'none';
  });
  // Lọc bảng bước 4
  document.querySelectorAll('#s4ticketList tbody tr[data-tid]').forEach(row => {
    const tid = (row.dataset.tid || '').toLowerCase();
    const isReplyRow = row.id && row.id.startsWith('reply-row-');
    const show = !q || tid.includes(q);
    if (isReplyRow) {
      if (!show) row.style.display = 'none';
    } else {
      row.style.display = show ? '' : 'none';
      const replyRow = document.getElementById('reply-row-' + row.dataset.tid);
      if (replyRow && !show) replyRow.style.display = 'none';
    }
  });
}
...
async function loadTickets() {
  ...
  for (const t of d.tickets) {
    const badge = t.replies_count > 0
      ? `<span class="badge badge-ok" style="cursor:pointer" onclick="toggleReplies('${t.id}',this)">${t.replies_count} reply ▾</span>`
      : `<span class="badge badge-skip">Chưa có</span>`;
    html += `<tr data-tid="${t.id}"><td>#${t.id}</td><td>${t.subject}</td><td style="font-size:12px">${t.cc_emails.join(', ')}</td><td>${badge}</td></tr>`;
    html += `<tr id="reply-row-${t.id}" data-tid="${t.id}" style="display:none"><td colspan="4" style="padding:0 8px 12px">
      <div id="reply-content-${t.id}" style="display:flex;flex-direction:column;gap:8px;margin-top:6px"></div>
    </td></tr>`;
  }
  html += '</tbody></table>';
...
async function toggleReplies(tid, btn) {
  const row = document.getElementById('reply-row-' + tid);
  const container = document.getElementById('reply-content-' + tid);
  ...
  container.innerHTML = '<span style="color:#64748b;font-size:12px">Đang tải...</span>';
  const r = await fetch('/mock-freshdesk/api/v2/tickets/' + tid + '/replies');
  const d = await r.json();
  if (!d.replies || d.replies.length === 0) {
    container.innerHTML = '<span style="color:#64748b;font-size:12px">Không có reply.</span>';
    return;
  }
  container.innerHTML = d.replies.map((rep, i) => {
    const ts = new Date(rep.timestamp).toLocaleString('vi-VN', {timeZone:'Asia/Ho_Chi_Minh'});
    const attachName = rep.attachment_name || 'Đính kèm';
    const attach = rep.has_attachment ? `<div><span class="preview-attach">📎 ${attachName}</span></div>` : '';
    return `<div class="preview-email" style="margin-bottom:0">
      <div class="meta"><b>Reply #${i+1}</b> · ${ts}<br>CC: ${rep.cc_emails.join(', ') || '—'}</div>
      ${rep.body || '(trống)'}
      ${attach}
    </div>`;
  }).join('');
...
async function sendFreshdesk() {
  ...
  btn.disabled = true;
  const tids = getCheckedTids();
  const total = tids.length;
  if (!total) return;

  // Hiện progress bar
  const progWrap = document.createElement('div');
  progWrap.id = 'send-progress-wrap';
  progWrap.style = 'margin:12px 0;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:8px;padding:12px 16px';
  progWrap.innerHTML = `
    <div style="font-size:13px;font-weight:600;color:#003087;margin-bottom:6px" id="send-prog-text">Đã gửi 0/${total} email</div>
    <div style="background:#e2e8f0;border-radius:6px;height:10px;overflow:hidden">
      <div id="send-prog-fill" style="height:100%;width:0%;background:#003087;transition:width 0.3s"></div>
    </div>`;
  document.getElementById('s3act').after(progWrap);

  const results = [];
  // Reset send_results trên server
  for (let i = 0; i < tids.length; i++) {
    const tid = tids[i];
    const r = await fetch('/send_freshdesk_one', {
      method: 'POST',
    ...
    document.getElementById('send-prog-text').textContent = `Đã gửi ${i+1}/${total} email`;
    document.getElementById('send-prog-fill').style.width = pct + '%';
  }
  await fetch('/send_freshdesk_done', {method:'POST'});
  progWrap.remove();
...
async function loadS4Tickets() {
  const r = await fetch('/mock-freshdesk/tickets');
  const d = await r.json();
  const sendResults = {};
  // map ticket_id -> status từ kết quả gửi
  document.querySelectorAll('#s4body tbody tr').forEach(row => {
    const tid = row.cells[0]?.textContent?.replace('#','').trim();
    const status = row.cells[3]?.textContent?.trim();
    if (tid) sendResults[tid] = status;
  });
  let html = '<table><thead><tr><th>Ticket ID</th><th>Chứng từ</th><th>CC</th><th>Trạng thái</th><th>Replies</th></tr></thead><tbody>';
  for (const t of d.tickets) {
    const status = sendResults[t.id] || '—';
    const statusBadge = status.includes('SUCCESS')
      ? `<span class="badge badge-ok">✓ Đã gửi</span>`
      : status === '—' ? `<span class="badge badge-skip">Chưa gửi</span>`
      : `<span class="badge badge-skip">${status}</span>`;
    const fname = (res?.files?.find(f => String(f.ticket_id) === String(t.id))?.file_name) || '';
    const fileCell = fname
      ? `<span class="preview-attach" style="cursor:pointer;font-size:12px" onclick="window.open('/view/${encodeURIComponent(fname)}','_blank')">📎 ${fname.replace(/[.]docx$/i,'.pdf')}</span>`
      : '—';
    const badge = t.replies_count > 0
      ? `<span class="badge badge-ok" style="cursor:pointer" onclick="toggleReplies4('${t.id}',this)">${t.replies_count} reply ▾</span>`
      : `<span class="badge badge-skip">Chưa có</span>`;
    html += `<tr data-tid="${t.id}"><td>#${t.id}</td><td>${fileCell}</td><td style="font-size:12px">${t.cc_emails.join(', ')}</td><td>${statusBadge}</td><td>${badge}</td></tr>`;
    html += `<tr id="reply-row-${t.id}" data-tid="${t.id}" style="display:none"><td colspan="5" style="padding:0 8px 12px">
      <div id="reply-content-${t.id}" style="display:flex;flex-direction:column;gap:8px;margin-top:6px"></div>
    </td></tr>`;
  }
  html += '</tbody></table>';
  document.getElementById('s4ticketList').innerHTML = html;
  document.getElementById('s4ticketWrap').style.display = '';
}

async function toggleReplies4(tid, btn) {
  const row = document.getElementById('reply-row-' + tid);
  const container = document.getElementById('reply-content-' + tid);
  if (row.style.display !== 'none') {
    row.style.display = 'none';
  ...
  container.innerHTML = d.replies.map((rep, i) => {
    const ts = new Date(rep.timestamp).toLocaleString('vi-VN', {timeZone:'Asia/Ho_Chi_Minh'});
    const attachName = rep.attachment_name || '';
    const attach = attachName
      ? `<div><span class="preview-attach" style="cursor:pointer" onclick="window.open('/view/${encodeURIComponent(attachName)}','_blank')">📎 ${attachName}</span></div>`
      : (rep.has_attachment ? `<div><span class="preview-attach">📎 Đính kèm</span></div>` : '');
    return `<div class="preview-email" style="margin-bottom:0">
      <div class="meta"><b>Reply #${i+1}</b> · ${ts}<br>CC: ${rep.cc_emails.join(', ') || '—'}</div>
      ${rep.body || '(trống)'}
      ${attach}
...
function filterS4() {
  const q = (document.getElementById('s4search').value || '').trim().toLowerCase();
  document.querySelectorAll('#s4ticketList tbody tr[data-tid]').forEach(row => {
    const tid = (row.dataset.tid || '').toLowerCase();
    const isReplyRow = row.id && row.id.startsWith('reply-row-');
    const show = !q || tid.includes(q);
    if (isReplyRow) { if (!show) row.style.display = 'none'; }
    else {
      row.style.display = show ? '' : 'none';
      const rr = document.getElementById('reply-row-' + row.dataset.tid);
...
@app.route("/send_freshdesk_one", methods=["POST"])
def send_freshdesk_one():
    """Gửi 1 ticket, dùng cho progress bar từng bước từ frontend."""
    import requests as req_lib
    base_url = "http://localhost:8080/mock-freshdesk"
    body = request.get_json(silent=True) or {}
    tid = str(body.get("ticket_id", "")).strip()

    item = next((x for x in state.get("freshdesk_data", []) if str(x.get("ticket_id","")).strip() == tid), None)
    if not item:
        return jsonify({"ticket_id": tid, "file_name": "", "cc": [], "status": "ERROR: không tìm thấy ticket"})
    ...
    try:
        data = [("body", """<div>Dear anh chị,<br><br>Zalopay cung cấp chứng từ khiếu nại như đính kèm. Xin cảm ơn.<br><br><span style="color:#003087"><b>Bộ phận vận hành Zalopay</b> · Operations Team · E: chargebackzp@vng.com.vn · P: (028) 3962 3888 ext:3227</span></div>"""),
                ("attachment_name", apath.name if apath.exists() else "")]
    ...
    result = {"ticket_id": tid, "file_name": fname, "cc": cc_final, "status": status}
    if "send_results" not in state:
        state["send_results"] = []
    state["send_results"].append(result)
    return jsonify(result)

@app.route("/send_freshdesk_done", methods=["POST"])
def send_freshdesk_done():
    """Lưu log sau khi frontend gửi hết."""
